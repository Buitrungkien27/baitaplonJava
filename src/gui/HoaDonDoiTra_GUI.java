...
public class HoaDonDoiTra_GUI extends javax.swing.JPanel {
...
//    private ReturnOrderManagament_BUS bus;
 ...
 // Hàm kiểm tra định dạng mã nhân viên
    private boolean isValidEmployeeID(String employeeID) {
        // Biểu thức chính quy để kiểm tra định dạng mã nhân viên NVabbbbccccxxx
        String regex = "^NV[01]\\d{4}\\d{4}\\d{3}$";
        return employeeID.matches(regex);
    }

    // Hàm phát sinh mã nhân viên tự động
    private String generateEmployeeID(int gender, int birthYear, int startYear, int sequenceNumber) {
        // Tạo mã nhân viên theo định dạng NVabbbbccccxxx
        // gender: 0 (nữ), 1 (nam)
        // birthYear: năm sinh (4 chữ số)
        // startYear: năm vào làm (4 chữ số)
        // sequenceNumber: số thứ tự, có 3 chữ số
        String genderStr = (gender == 1) ? "1" : "0";
        String birthYearStr = String.format("%04d", birthYear);
        String startYearStr = String.format("%04d", startYear);
        String sequenceStr = String.format("%03d", sequenceNumber);

        return "NV" + genderStr + birthYearStr + startYearStr + sequenceStr;
    }

    // Lấy số thứ tự tiếp theo cho mã nhân viên
    private int getNextSequenceNumber() {
        // Tăng số thứ tự dựa trên số lượng nhân viên hiện có trong bảng
        return tblModel_returnOrder.getRowCount() + 1;
    }

    public HoaDonDoiTra_GUI() {
        initComponents();
        init();
    }
    private void init(){
        ...
        tblModel_product = new DefaultTableModel(new String[]{"Mã vé tàu", "Số lượng"}, 0);
        ...
        // Đảm bảo tất cả các JTextField có thể chỉnh sửa
